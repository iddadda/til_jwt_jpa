...
public class JwtTokenProvider {
    ...
    public String generateToken(JwtUser jwtUser, int tokenValidityMilliSeconds) {
        Date now = new Date();     // Date 객체를 기본 생성자로 만들면 현재 일시 정보로 객체화됨
        return Jwts.builder()
//        header
...
//        try 에서 예외 발생 시 catch 문 실행이 된 채로 계속 실행됨
        try {
    ...
    public JwtUser getJwtUserFromToken(String token) {
        ...
        String json = claims.get(constJwt.getClaimKey(), String.class);
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "토큰에 문제");
        }
    }
